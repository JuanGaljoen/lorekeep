...
*Done when:* every test gating this change is either vetted here or already on record as trusted.
...
*Done when:* every success criterion carries its own evidence — a command's output, a rendered
screen, an observation you collected. A criterion with no artifact behind it is unchecked, however
confident the reading of the code.
...
*Done when:* every constant the change introduced or moved has a named source, and every smell in
[SMELLS.md](SMELLS.md) has been weighed against the diff.
